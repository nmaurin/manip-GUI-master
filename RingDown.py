...
class MainWindow(QtWidgets.QMainWindow): 
    ...
    def catch_param_change(self, _, changes):
        ...
        for param, _, data in changes:
            ...
            ## START ACQUISITION
            if (who+'.'+what) in 'AcquisitionRD.start':
                # earase data
    ...
    def ringdown(self):
        time1 = self.parameters.give('AcquisitionRD','excitation time')
        time2 = self.parameters.give('AcquisitionRD','de-excitation time')
        time0 = time.time() # computer time in s      
        # update generator value on the instrument
        instru = self.parameters.give('Generator','instrument')
        gene_value = self.parameters.give_all_inst_value('Generator')
        for i in range(0,len(gene_value)):
            self.instruments.set_value(instru,gene_value[i][0],gene_value[i][1])
        # get X Y R and Phi from instrument
        temp = self.instruments.get_X_Y_R_Phi(instru,time2)
        # update data
        graph_1 = self.obj_graph['plot']['curve']
...
class MesParams():
    def __init__(self):
        ...
        self.dico = {
            ...
            'AcquisitionRD':toolbox.parameter_tree.AcquisitionRD(name='AcquisitionRD'),
            'FitRD':toolbox.parameter_tree.FitRD(name='FitRD'),
    ...
    def give_all_inst_value(self,who):
        ...
        if who in 'Generator':
            for k in ['frequency','amplitude','on']: 
                A.append([k,self.dico[who].param(k).value()])
        return A
